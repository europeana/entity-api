...
public class ConceptSchemeController extends BaseRest {
	...
	try {
	    ...
	    List<EntityTypes> entityTypes = queryBuilder.extractEntityTypes(parameters);
	    //we need to process entity types, for building the correct filter
	    getEntityService().validateEntityTypes(entityTypes, false);
	    String scope = queryBuilder.extractScope(parameters);
	    ...
	    getLogger()
		    .debug("Matching concepts for scheme : " + identifier + ", #results: " + matchingEntityIds.size());
	    ...
	    Query existingQuery = queryBuilder.buildSearchQuery(inSchemeQuery, null, 0);
	    //TODO: update logic for buildSearchQuery to handle internal and external max page sizes
	    ...
	    ResultSet<? extends Entity> storedResults = getEntityService().search(existingQuery, null, entityTypes,
		    scope);
    ...
    private List<String> searchEntityIds(Query searchQuery, String scope, List<EntityTypes> entityTypes)
	    throws HttpException {
